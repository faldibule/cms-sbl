...
import { POSupplierCateringDummy, dummy_item_product } from '@utils/Dummy'
...
const Form = (props) => {
    ...
    // PR Handle
    const [poSupplierCatering, setPoSupplierCatering] = useState({
        input: '',
        selected: ''
    })
    const handleSelectedPOSupplierCatering = (value) => setPoSupplierCatering({...poSupplierCatering, selected: value})
    const handleInputPOSupplierCatering = (value) => setPoSupplierCatering({...poSupplierCatering, input: value})
    // const { data: dataPOSupplierCatering, isLoading: loadingPOSupplierCatering } = useFetchPurchaseRequest({ paginate: 0, status: ['finish'] })
    // const { data: dataPOSupplierCateringById, isLoading: loadingPrById } = useFetchPurchaseRequestById(poSupplierCatering.selected?.id, { enabled: !!poSupplierCatering.selected?.id })

    // useEffect(() => {
    //     let mounted = true
    //     if(!!!poSupplierCatering.selected?.id) return
    //     if(!!!dataPOSupplierCateringById) return
    //     if(!mounted) return 

    //     if(props.title === 'edit' && poSupplierCatering.selected?.id === data.purchase_request?.id){
    //         setItem([...data.item_product])
    //         return
    //     }
        
    //     setItem([...dataPOSupplierCateringById.item_product])

    //     return () => mounted = false

    // }, [poSupplierCatering.selected, dataPOSupplierCateringById])
                ...
                <Card sx={{ p: 2, mt: 3 }}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={12}>
                            <CustomAutocomplete 
                                disabled={isApproved}
                                options={POSupplierCateringDummy}
                                getOptionLabel={(option) => `${option.po_supplier_catering_number}`}
                                label='PO Supplier Catering'
                                inputValue={poSupplierCatering.input}
                                setInputValue={handleInputPOSupplierCatering}
                                selectedValue={poSupplierCatering.selected}
                                setSelectedValue={handleSelectedPOSupplierCatering}
                                errors={errors?.purchase_request_id}
                                key='PR'
                            /> 
                        </Grid>
                        {!!poSupplierCatering.selected?.id ?
                        <>
                        <Grid item xs={12} md={12}>
                            <TextField 
                                label='Supplier'
